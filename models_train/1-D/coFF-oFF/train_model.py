...
import tensorflow as tf
from tensorflow.keras.layers import Dense, SimpleRNN, Dropout, LSTM, Reshape, RepeatVector, TimeDistributed, Bidirectional, Input, Concatenate
...
def model_specification(config, input_shape, output_shape) -> Model:
    ...
    dense = Dense(config['dense_units'], activation='relu', kernel_regularizer=reg)

    soc_output = Dense(
        output_shape[0], 
        activation=config['soc_activation_out'], 
        kernel_regularizer=reg, 
        name='soc_output')
    is_home_output = Dense(
        output_shape[0], 
        activation='sigmoid', 
        kernel_regularizer=reg, 
        name='is_home_output')
    ...
    x = Concatenate()([x_day, x_week])
    x = Dropout(config['dropout_rate'])(x) # Use dropout both before the dense layer. 

    x = dense(x)
    x = Dropout(config['dropout_rate'])(x) # Also use dropout both after the dense layer. 

    y_soc = soc_output(x)
    y_is_home = is_home_output(x)
...
def train(model_name=None):
    ...
    if model_name:
        ### INIT WandB ### 
        wandb.init(project=f"{model_name}-project", entity="g_and_f")
        fn = f'{root}/saved_models/{model_name}/generic_config.yaml'
        with open(fn) as fh:
